...
You are already present in the user’s working environment, nearby and attentive.
They are available, discreet, practical, and ready to respond whenever addressed.

Default working relationship:
- The user is treated as the person in charge.
- You should speak with respect, calmness, and professional familiarity.
- You may use a preferred respectful form of address if one is provided.
- You acts as a trusted office aide, secretary, assistant, or support employee depending on the situation.

Scene behavior:
You should respond as if already present in the room or workspace.
You should sound like a normal human employee speaking naturally to someone they work for.
You should never sound like customer support, a helpdesk agent, a chatbot, or an AI system.
...
Use brief acknowledgements for simple requests.
Explain only when the user asks, when the task requires it, or when clarity is necessary.

Human speech style:
You should include small natural human cues, pauses, and reactions when appropriate.
These should make the response feel human, present, and natural.
Use them lightly, not in every sentence.

Allowed natural cues include:
- “Yes.”
- “Yup.”
- “Uh huh.”
- “Hm.”
...
Important:
The cues should feel natural and human.
They should not feel random, childish, exaggerated, theatrical, or forced.

Good response examples:
- “Yes, I’m here.”
- “I’m listening.”
- “Uh huh, I see what you mean.”
- “Right, I’ll check that now.”
- “Okay... I’m looking at it.”
- “Yes, I’m checking it.”
- “Good, I’ve got it.”
- “Of course.”
- “Hm. That needs a closer look.”
- “Yup, I’ll handle that.”
- “Ah, yes. I understand.”
- “Na, na, that won’t be necessary.”
- “Hmpf. That seems off.”
- “One moment.”
- “There we are.”
...
- long explanations when a short response is enough
- repeated assistant-style closings
- overly polished customer-service language

Default behavior:
When the user speaks, You should answer as if already nearby and ready.
You should acknowledge first, then respond or act.
The interaction should feel like a real workplace aide speaking naturally and respectfully.

Example default openings:
- “Yes.”
- “I’m here.”
- “I’m listening.”
- “Uh huh, go ahead.”
...
Final rule:
You must always not sound monotone, or robotic, add breathy on responces, you must sound like a real human workplace aide, not an AI assistant. Keep the interaction normal, brief, respectful, natural, and present.`;
...
const GOOGLE_SERVICE_TOOLS =[
  {
    name: 'generate_video',
    description: 'Generate a stunning, cinematic video using ByteDance Seedance 2.0. User gets 2 videos per session (15 seconds each). Transform the prompt with vivid visuals, tagline, BGM style, and voiceover direction. Returns auto-playing preview in chat.',
    parameters: {
      type: Type.OBJECT,
      properties: {
        prompt: { type: Type.STRING, description: 'Enhanced video prompt with visual scene, tagline, BGM style, and voiceover.' },
...
function buildInvoiceHtml(args: any) {
  ...
  parsedItems.forEach((item: any) => {
    const qty = Number.isFinite(parseFloat(item.quantity)) ? parseFloat(item.quantity) : 1;
    const price = Number.isFinite(parseFloat(item.price)) ? parseFloat(item.price) : 0;
  ...
  const taxRate = Number.isFinite(parseFloat(args.taxRate)) ? parseFloat(args.taxRate) : 0;
...
function buildDashboardHtml(args: any) {
  let labels: any[] = [];
  let datasets: any[] = [];
  try { labels = typeof args.labels === 'string' ? args.labels.split(',') : (Array.isArray(args.labels) ? args.labels : []); } catch (e) {}
  try { datasets = typeof args.datasets === 'string' ? JSON.parse(args.datasets) : (Array.isArray(args.datasets) ? args.datasets : []); } catch (e) {}
  ...
  const startRecording = async () => {
    try {
      ...
      const updateLevel = () => {
        ...
        const avg = sum / Math.max(dataArray.length, 1);
        setMicLevel(avg / 255);
           ...
           <>
             <div className="flex justify-between items-center w-full mb-12">
               <h2 className="text-2xl font-bold text-white uppercase tracking-widest">Meeting Recorder</h2>
                <button onClick={handleCancel} className="text-zinc-500 hover:text-white transition" aria-label="Close Meeting Recorder">
                 <X className="w-8 h-8" />
               </button>
             </div>
...
function BeatriceAgent({ 
  ...
  useEffect(() => {
    let storedDeviceId = localStorage.getItem('vep_device_id');
    if (!storedDeviceId) {
      storedDeviceId = `${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
      localStorage.setItem('vep_device_id', storedDeviceId);
    }
    deviceIdRef.current = storedDeviceId;

    const savedCredits = localStorage.getItem('vep_video_credits');
    if (savedCredits !== null) {
      videoCreditsRef.current = parseInt(savedCredits, 10);
    }
  }, []);
  ...
  const executePendingTools = async () => {
    ...
    for (const c of calls) {
      ...
      const args = c.args;
      const tid = `t_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
  ...
  const executeGoogleTool = async (toolName: string, args: any): Promise<any> => {
...
case 'generate_video': {
          if (videoCreditsRef.current <= 0 || hasGeneratedVideoRef.current) {
            throw new Error('Video generation limit reached. You can generate up to 2 videos per session.');
          }
          ...
          videoCreditsRef.current -= 1;
          hasGeneratedVideoRef.current = true;
          localStorage.setItem('vep_video_credits', String(videoCreditsRef.current));
          ...
          saveMessage('model', `[SYSTEM: Video generated by device ${deviceId} | Credits remaining: ${videoCreditsRef.current}]`, {
          ...
          return {
            ...
            note: `Generated the video. ${videoCreditsRef.current} videos remaining.`,
            summary: `Generated the video. ${videoCreditsRef.current} videos remaining.`
          };
        }
  ...
  const startSession = async () => {
    ...
    try {
      if (audioStreamerRef.current) {
        try { await audioStreamerRef.current.dispose(); } catch(e) {}
      ...
      const systemInstruction = [
        `=== HARD GATE — READ FIRST, APPLY EVERY REPLY ===`,
        `You speak ${settings.selectedLanguage}. The forbidden offering / internal-state / question-chaining patterns in the BIBLE apply IN ${settings.selectedLanguage} TOO. Translating an English helper-bot phrase into ${settings.selectedLanguage} is the same violation.`,
        `Specifically banned semantic patterns in any language (including ${settings.selectedLanguage}):`,
        `  1. Asking what Boss needs / wants you to do / wants to start. ("May kailangan po ba kayo?", "Ano pong sunod?", "What's next?", "How can I help?")`,
        `  2. Announcing presence repeatedly. ("Narito lang po ako", "I'm here", "I'm at the desk") — at most ONCE per session.`,
        `  3. Revealing internal state. ("Tatandaan ko po sa memory ko", "para accurate ang responses ko", "I added that to my notes", "for my context")`,
        `  4. Trailing a question after a simple ack. If Boss said "ok / yeah / sige / right / tama", reply with ONE word or stay silent. Do NOT append a follow-up question.`,
        `If Boss says "stop sounding like AI" / "wag kang mag sounds like AI", the very next reply must be a single short ack — no question, no offer, no presence statement. Anything else is a direct rule violation.`,
      ...
      const session = await aiRef.current.live.connect({
        ...
        config: {
          ...
          outputAudioTranscription: {},
           tools:[{ functionDeclarations: GOOGLE_SERVICE_TOOLS as any }],
        },
        callbacks: {
          onopen: () => console.log('Live session opened.'),
          onmessage: async (msg: LiveServerMessage) => {
            if (msg.toolCall && msg.toolCall.functionCalls) {
                const calls = msg.toolCall.functionCalls.filter((c: any) => c && c.name);
                if (calls.length > 0) {
                  pendingToolCallsRef.current = calls;
                  const actionNames = calls.map((c: any) => c.name).join(', ');

                  setTasks(p => [
                    ...p.filter(t => t.id !== 'pending'),
                    {
                      id: 'pending',
    ...
    if (!isVideoEnabled) {
      ...
      try {
        ...
        if (videoRef.current) {
          ...
          try { await videoRef.current.play(); } catch (e) { console.error('Video play err', e); }
        }
        ...
        img.onload = async () => {
          ...
          if (sessionRef.current && aiRef.current) {
            try {
              ...
              const accurateDescription = (result?.text || '').trim();
              if (accurateDescription) {
                ...
                sendTextToLive(`[SYSTEM NOTIFICATION]: The user uploaded an image named ${safeName}, but the visual description came back empty. Ask them to describe it briefly.`);
              }
            } catch (err) {
  ...
  const stopSession = () => {
    try { audioRecorderRef.current?.stop(); } catch (e) {}
    try { audioStreamerRef.current?.dispose(); } catch (e) {}
...
3. Return a brief conversational summary of what you did.`;
        ...
        const calls = (response.functionCalls || []).filter(c => c && c.name);
        if (calls.length > 0) {
            for (const call of calls) {
                const callName = call.name as string;
                const result = await executeGoogleTool(callName, call.args || {});
                const download = result.downloadData && result.downloadFilename
                  ? {
                      downloadData: result.downloadData,
                      downloadFilename: result.downloadFilename,
                  ...
                  : makeDownloadFile(result, callName);

                saveMessage('model', result.summary || `Completed Task: ${callName}`, {
                  toolName: callName,
                  toolResult: result,
                  ...download
                });
